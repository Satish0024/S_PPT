...
// page's "Generate Statement" action -- same sessionStorage pattern as
...
// genuine failure surfaces as the error toast rather than a blank document
...
export function addGeneratedStatement(participantId, { planName, periodLabel }) {
  ...
  const doc = {
    id: `gen-${Date.now()}`,
    name: `${planName} Statement_${new Date()
