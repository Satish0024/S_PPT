...
export function elections(participantId) {
  // Prefer per-participant map (new enrollment writes); fall back to legacy
  // global session shape so older demo sessions still hydrate.
  const map = readMap(DEFERRAL_KEY)
  const deferral = (participantId && map[participantId]) || (map.pre != null || map.roth != null ? map : null) || readSession(DEFERRAL_KEY)
  const autoInc = readSession(AUTO_INCREASE_KEY)
  const optedOut = !!(deferral?.optedOut || deferral?.mode === 'optout')
  const pre = optedOut ? 0 : Number.isFinite(+deferral?.pre) ? +deferral.pre : 6
  const roth = optedOut ? 0 : Number.isFinite(+deferral?.roth) ? +deferral.roth : 2
  // New shape stores autoOn/autoPct on the deferral record; legacy used AUTO_INCREASE_KEY.
  const autoOn = !optedOut && (
    typeof deferral?.autoOn === 'boolean'
      ? deferral.autoOn
      : autoInc?.mode === 'do' && !autoInc?.skipped
  )
  const autoPct = Number.isFinite(+deferral?.autoPct)
    ? +deferral.autoPct
    : Number.isFinite(+autoInc?.incPre)
      ? +autoInc.incPre
      : Number.isFinite(+autoInc?.inc)
        ? +autoInc.inc
        : 1
  const autoCap = Number.isFinite(+deferral?.autoCap)
    ? +deferral.autoCap
    : Number.isFinite(+autoInc?.capPre)
      ? +autoInc.capPre
...
export function defaultPrefs(participant) {
  ...
  return { location, retireAge: 67, ...LOCATION_DEFAULTS[location], ...elections(participant?.id) }
}
